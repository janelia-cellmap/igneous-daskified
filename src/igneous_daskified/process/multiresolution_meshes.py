...
from ..util import multiresolution_mesh_util, io_util, dask_util
import logging
...
def update_fragment_dict(dictionary, fragment_pos, vertices, faces, lod_0_fragment_pos):
    ...
    if fragment_pos in dictionary:
        ...
        dictionary[fragment_pos] = multiresolution_mesh_util.Fragment(
...
def generate_mesh_decomposition(
    ...
    vertices, faces = multiresolution_mesh_util.mesh_loader(mesh_path)
    ...
    for fragment_pos, fragment in combined_fragments_dictionary.items():
        ...
        if len(draco_bytes) > 12:
            ...
            fragment = multiresolution_mesh_util.CompressedFragment(
                draco_bytes,
...
def pyfqmr_decimate(
    ...
    vertices, faces = multiresolution_mesh_util.mesh_loader(f"{input_path}/{id}{ext}")
...
def generate_neuroglancer_multires_mesh(
    ...
    with ExitStack() as stack:
        ...
        for idx, current_lod in enumerate(lods):
            ...
            vertices, _ = multiresolution_mesh_util.mesh_loader(mesh_path)
            ...
            if vertices is not None:
                ...
                multiresolution_mesh_util.write_mesh_files(
    ...
    with io_util.tee_streams(logpath):
        ...
        try:
            ...
            with io_util.Timing_Messager(
                ...
                multiresolution_mesh_util.write_segment_properties_file(
                    multires_output_path
                )
                multiresolution_mesh_util.write_info_file(multires_output_path)
